...
class App extends React.Component{
    constructor(){
        ...
        this.state = {
          ...
          matches: [
            { player1: "Andre", player2: "Walmir", played: false },
            { player1: "Thiago", player2: "Daniel", played: false },
          ],
  ...
  addMatch = (player1, player2) => {
    const newMatch = { player1: player1, player2: player2, played: false }
    this.setState({ matches: this.state.matches.concat(newMatch) })
  }
          ...
          <ol>
            {this.state.matches.map(match => (
              <li key={match.player1+match.player2}>
                <span>
                  {match.player1}
                  <button disabled={match.played === true} onClick={() => this.play(match, match.player1)}>Win!</button>
                </span>
                <span> VS </span>
